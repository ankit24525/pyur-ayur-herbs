...
export async function POST(request: Request) {
  try {
    ...
    console.log("[Delivery Webhook Event Received]:", body);
    ...
    if (orderIndex !== -1) {
      ...
      await writeDB(db);
      console.log(`[Delivery Webhook Updated]: Order ${order_id} -> Status: ${mappedStatus} (${current_status})`);
      ...
      if (whatsappToken && whatsappPhoneId && order.phone) {
        try {
          ...
          console.error("[Delivery Webhook WhatsApp Trigger Error]:", wErr);
        }
      }
    }
