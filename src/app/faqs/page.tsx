...
import { defaultFaqs as DEFAULT_FAQS } from "@/lib/default-faqs";

const isLegacyFaq = (f: any): boolean => {
...
export default function FaqsPage() {
  ...
  useEffect(() => {
    // 1. Check localStorage cache first with strict legacy filter to prevent old data flash
    if (typeof window !== "undefined") {
      try {
        const cached = JSON.parse(localStorage.getItem("pyur_storefront_cache") || "{}");
        if (Array.isArray(cached.faqs) && cached.faqs.length > 0) {
          const cleanCached = cached.faqs.filter((f: any) => !isLegacyFaq(f));
    ...
    // 2. Fetch live data from storefront client
    getStorefrontData(true)
      .then((data) => {
        if (Array.isArray(data?.faqs) && data.faqs.length > 0) {
          const cleanServerFaqs = data.faqs.filter((f: any) => !isLegacyFaq(f));
          if (cleanServerFaqs.length > 0) {
            setFaqs(cleanServerFaqs);
          } else {
            setFaqs(DEFAULT_FAQS);
          }
        }
    ...
    // 3. Listen for realtime admin edits
    const handleUpdate = (e: any) => {
      if (e.detail?.key === "faqs" && Array.isArray(e.detail.value)) {
        const clean = e.detail.value.filter((f: any) => !isLegacyFaq(f));
        if (clean.length > 0) {
          setFaqs(clean);
        }
      }
